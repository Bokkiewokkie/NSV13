...
/datum/starsystem_manager/ui_interact(mob/user, datum/tgui/ui)
	if(!SSstar_system?.systems)
		return to_chat(user, "<span class='warning'>Starsystems haven't been initialized yet!</span>")
...
/datum/starsystem_manager/ui_data(mob/user)
	...
	for(var/datum/star_system/SS in SSstar_system.systems)
		...
		sys_inf["owner"] = capitalize(SS.owner)
...
/client/proc/instance_overmap_menu()
